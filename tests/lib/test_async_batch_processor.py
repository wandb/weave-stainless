...
from weave_server_sdk.lib.async_batch_processor import AsyncBatchProcessor


def test_max_batch_size() -> None:
    processor_fn = MagicMock()
    processor = AsyncBatchProcessor[int](processor_fn, max_batch_size=2)
...
def test_min_batch_interval() -> None:
    processor_fn = MagicMock()
    processor = AsyncBatchProcessor[int](processor_fn, max_batch_size=100, min_batch_interval=1)
...
def test_wait_until_all_processed() -> None:
    processor_fn = MagicMock()
    processor = AsyncBatchProcessor[int](processor_fn, max_batch_size=100, min_batch_interval=0.01)
...
def test_health_check_thread_is_created() -> None:
    """Test that health check thread is created and running."""
    processor_fn = MagicMock()
    processor = AsyncBatchProcessor[int](processor_fn, max_batch_size=100, min_batch_interval=0.1)
...
def test_health_check_functionality_and_logging() -> None:
    """Test health check revival functionality, logging, and full queue behavior."""
    processor_fn = MagicMock()

    with patch("weave_server_sdk.lib.async_batch_processor.logger") as mock_logger:
        # Create processor with small queue to test full queue behavior
        processor = AsyncBatchProcessor[int](processor_fn, max_batch_size=100, min_batch_interval=0.1, max_queue_size=1)
...
def test_processing_thread_exception_handling() -> None:
    ...
    def failing_processor_fn(_batch: List[int]) -> None:
        raise RuntimeError("Simulated processing error")

    with patch("weave_server_sdk.lib.async_batch_processor.logger") as mock_logger:
        processor = AsyncBatchProcessor[int](
        ...
        # Stop and check logs
        processor.stop_accepting_new_work_and_flush_queue()
        assert "Unprocessable item detected, dropping item permanently." in mock_logger.error.call_args[0][0]
...
def test_poison_pill_detection_and_immediate_drop() -> None:
    ...
    def selective_failing_processor(batch: List[str]) -> None:
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        processor = AsyncBatchProcessor[str](
        ...
        processor_no_disk = AsyncBatchProcessor[str](
            selective_failing_processor,
...
def test_log_rotation_and_disk_fallback() -> None:
    """Test log file rotation, backup management, and disk operation error handling."""

    def simple_processor(batch: List[Any]) -> None:
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with patch(
            "weave_server_sdk.lib.async_batch_processor.MAX_LOG_FILE_SIZE_BYTES",
            200,
        ), patch("weave_server_sdk.lib.async_batch_processor.MAX_LOGFILES", 2):
            processor = AsyncBatchProcessor[str](
                simple_processor,
                enable_disk_fallback=True,
                disk_fallback_path=str(log_path),
            ...
            _backup2_path = log_path.with_suffix(".2")
            ...
            with patch("builtins.open", side_effect=PermissionError("No write access")), patch(
                "weave_server_sdk.lib.async_batch_processor.logger"
            ) as mock_logger:
                processor._write_item_to_disk("test_item", "Permission test")

                # Should log the disk write failure
                mock_logger.error.assert_called_with(
                    f"Failed to write dropped item {id('test_item')} to disk: No write access", exc_info=mock.ANY
                )
            ...
            with patch("pathlib.Path.rename", side_effect=OSError("Rename failed")), patch(
                "weave_server_sdk.lib.async_batch_processor.logger"
            ) as mock_logger:
            ...
            processor_nested = AsyncBatchProcessor[str](
                simple_processor,
                enable_disk_fallback=True,
