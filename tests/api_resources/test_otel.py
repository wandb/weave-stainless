...
class TestOtel:
    ...
    def test_method_export(self, client: WeaveTrace) -> None:
        ...
        assert_matches_type(object, otel, path=["response"])
    ...
    def test_streaming_response_export(self, client: WeaveTrace) -> None:
        with client.otel.with_streaming_response.export() as response:
...
class TestAsyncOtel:
    ...
    async def test_streaming_response_export(self, async_client: AsyncWeaveTrace) -> None:
        async with async_client.otel.with_streaming_response.export() as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"
